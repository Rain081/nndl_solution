...
class RNN_model(nn.Module):
    ...
    def forward(self,sentence,is_test = False):
        ...
        # ???
        ################################################
        seq_len = batch_input.size(1)    # 获取序列长度
        
        # 初始化隐藏状态和细胞状态 (num_layers, batch, hidden_dim)
        # 设置为零初始化
        # h0 = torch.zeros(2, 1, self.lstm_dim)      # 2层LSTM，batch_size=1
        # c0 = torch.zeros(2, 1, self.lstm_dim)

        # LSTM前向传播
        # output: (batch, seq_len, hidden_dim)
        # (hn, cn): 最后时刻的隐藏状态和细胞状态
        output, _ = self.rnn_lstm(batch_input, None)  # 隐藏状态和细胞状态自动初始化为零
        # output shape: (1, seq_len, lstm_dim) 将输出重塑为 (seq_len, lstm_dim) 以便全连接层处理
